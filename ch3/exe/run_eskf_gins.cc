...
int main(int argc, char** argv) {
    ...
    // 初始化器
    pj::StaticIMUInit imu_init;  // 使用默认配置
    pj::ESKFD eskf;

    pj::TxtIO io(FLAGS_txt_path);
    ...
    auto save_result = [&save_vec3, &save_quat](std::ofstream& fout, const pj::NavStated& save_state) {
    ...
    io.SetIMUProcessFunc([&](const pj::IMU& imu) {
          /// IMU 处理函数
        ...
        .SetGNSSProcessFunc([&](const pj::GNSS& gnss) {
            /// GNSS 处理函数
            if (!imu_inited) {
                return;
            }

            pj::GNSS gnss_convert = gnss;
            if (!pj::ConvertGps2UTM(gnss_convert, antenna_pos, FLAGS_antenna_angle) || !gnss_convert.heading_valid_) {
                return;
            }
        ...
        .SetOdomProcessFunc([&](const pj::Odom& odom) {
