...
bool ConvertGps2UTM(GNSS& gps_msg, const Vec2d& antenna_pos, const double& antenna_angle, const Vec3d& map_origin) {
    ...
    if (gps_msg.heading_valid_) {
        heading = (90 - gps_msg.heading_) * kDEG2RAD;  // 北东地转到东北天
    }

    /// TWG 转到 TWB---body就是车体中心，也是副天线，G是主天线
    SE3 TBG(SO3::rotZ(antenna_angle * kDEG2RAD), Vec3d(antenna_pos[0], antenna_pos[1], 0));
    SE3 TGB = TBG.inverse();
