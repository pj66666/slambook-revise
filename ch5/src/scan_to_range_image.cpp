...
void GenerateRangeImage(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud) {
    ...
    for (const auto& pt : cloud->points) {
        double azimuth = atan2(pt.y, pt.x) * 180 / M_PI;    // 方位角
        double range = sqrt(pt.x * pt.x + pt.y * pt.y);     // 距离
        double elevation = asin((pt.z - FLAGS_lidar_height) / range) * 180 / M_PI;  // 俯仰角
